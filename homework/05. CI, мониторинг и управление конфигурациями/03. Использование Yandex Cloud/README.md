# Домашнее задание по теме: "Использование Yandex Cloud"
...
1. Допишите playbook: нужно сделать ещё один play, который устанавливает и настраивает lighthouse.
    ...
    **Результат:**
    ...
    Весь playbook доступен в [репозитории](https://github.com/ivanmanokhin/cvl-stack/blob/main/playbook/site.yml).

2. При создании tasks рекомендую использовать модули: `get_url`, `template`, `yum`, `apt`.
3. Tasks должны: скачать статику lighthouse, установить nginx или любой другой webserver, настроить его конфиг для открытия lighthouse, запустить webserver.
   
   Выполнено. Tasks выполняют данные действия.
...
5. Запустите `ansible-lint site.yml` и исправьте ошибки, если они есть.
    ...
    **Результат:**
    ...
    ```bash
    /playbook(main)]> ansible-lint site.yml 
...
6. Попробуйте запустить playbook на этом окружении с флагом `--check`.
    ...
    **Результат:** Check не проходит т.к. playbook не может продолжить проверку без скачанного rpm-пакета ClickHouse.
    ...
    failed: [clickhouse] (item=clickhouse-client) => {"ansible_loop_var": "item", "changed": false, "dest": "./clickhouse-client-22.9.7.34.rpm", "elapsed": 0, "item": "clickhouse-client", "msg": "Request failed", "response": "HTTP Error 404: Not Found", "status_code": 404, "url": "https://packages.clickhouse.com/rpm/stable/clickhouse-client-22.9.7.34.noarch.rpm"}
    failed: [clickhouse] (item=clickhouse-server) => {"ansible_loop_var": "item", "changed": false, "dest": "./clickhouse-server-22.9.7.34.rpm", "elapsed": 0, "item": "clickhouse-server", "msg": "Request failed", "response": "HTTP Error 404: Not Found", "status_code": 404, "url": "https://packages.clickhouse.com/rpm/stable/clickhouse-server-22.9.7.34.noarch.rpm"}
    failed: [clickhouse] (item=clickhouse-common-static) => {"ansible_loop_var": "item", "changed": false, "dest": "./clickhouse-common-static-22.9.7.34.rpm", "elapsed": 0, "item": "clickhouse-common-static", "msg": "Request failed", "response": "HTTP Error 404: Not Found", "status_code": 404, "url": "https://packages.clickhouse.com/rpm/stable/clickhouse-common-static-22.9.7.34.noarch.rpm"}
    ...
    fatal: [clickhouse]: FAILED! => {"changed": false, "msg": "No RPM file matching 'clickhouse-common-static-22.9.7.34.rpm' found on system", "rc": 127, "results": ["No RPM file matching 'clickhouse-common-static-22.9.7.34.rpm' found on system"]}
...
7. Запустите playbook на `prod.yml` окружении с флагом `--diff`. Убедитесь, что изменения на системе произведены.
    ...
    **Результат:**
    ...
    <details> 
      ...
      TASK [Generate ClickHouse config] *****************************************************************************************************************************************
      --- before: /etc/clickhouse-server/config.xml
      +++ after: /home/ivanm/.ansible/tmp/ansible-local-1029c7djvh1b/tmpct6srhth/config.xml.j2
      @@ -1,722 +1,87 @@
      -<!--
      ...
      TASK [Generate ClickHouse users config] ***********************************************************************************************************************************
      --- before: /etc/clickhouse-server/users.xml
      +++ after: /home/ivanm/.ansible/tmp/ansible-local-1029c7djvh1b/tmphegpcs5i/users.xml.j2
      @@ -1,110 +1,31 @@
       <clickhouse>
      ...
      PLAY [Create db & user in ClickHouse (for Vector)] ************************************************************************************************************************
      
      TASK [Gathering Facts] ****************************************************************************************************************************************************
      ok: [clickhouse]
      
      TASK [Create database "logs"] *********************************************************************************************************************************************
      changed: [clickhouse]
      ...
      TASK [Create directory for Vector data] ***********************************************************************************************************************************
      --- before
      +++ after
      @@ -1,6 +1,6 @@
       {
      ...
      TASK [Generate systemd unit (Vector)] *************************************************************************************************************************************
      --- before
      +++ after: /home/ivanm/.ansible/tmp/ansible-local-1029c7djvh1b/tmpr1glg2yb/vector.service.j2
      @@ -0,0 +1,18 @@
      +[Unit]
      +Description=Vector
      ...
      --- before
      +++ after: /home/ivanm/.ansible/tmp/ansible-local-1029c7djvh1b/tmp4cugf369/vector.yml.j2
      @@ -0,0 +1,24 @@
      ...
      +    endpoint: http://51.250.8.183:8123
      +    inputs:
      +    - logs
      ...
      TASK [Set SELinux Type] ***************************************************************************************************************************************************
      # Addition to semanage file context mappings
      +/usr/share/nginx/html/lighthouse      a      system_u:object_r:httpd_sys_content_t:s0
      changed: [lighthouse]
      
      TASK [Generate Nginx (Lighthouse) Config] *********************************************************************************************************************************
      --- before
      +++ after: /home/ivanm/.ansible/tmp/ansible-local-1029c7djvh1b/tmpmlvze7no/lighthouse.j2
      @@ -0,0 +1,6 @@
      +server {
      ...
      RUNNING HANDLER [Start Nginx server] **************************************************************************************************************************************
      changed: [lighthouse]
      
      PLAY RECAP ****************************************************************************************************************************************************************
      clickhouse                 : ok=9    changed=7    unreachable=0    failed=0    skipped=0    rescued=1    ignored=0   
      lighthouse                 : ok=8    changed=7    unreachable=0    failed=0    skipped=0    rescued=0    ignored=0   
      vector                     : ok=9    changed=8    unreachable=0    failed=0    skipped=0    rescued=0    ignored=0   
      ```
      </details>

8. Повторно запустите playbook с флагом `--diff` и убедитесь, что playbook идемпотентен.

    **Результат:** playbook идемпотентен
    ...
    TASK [Get ClickHouse distrib] *********************************************************************************************************************************************
    failed: [clickhouse] (item=clickhouse-client) => {"ansible_loop_var": "item", "changed": false, "dest": "./clickhouse-client-22.9.7.34.rpm", "elapsed": 0, "gid": 1000, "group": "centos", "item": "clickhouse-client", "mode": "0644", "msg": "Request failed", "owner": "centos", "response": "HTTP Error 404: Not Found", "secontext": "unconfined_u:object_r:user_home_t:s0", "size": 88071, "state": "file", "status_code": 404, "uid": 1000, "url": "https://packages.clickhouse.com/rpm/stable/clickhouse-client-22.9.7.34.noarch.rpm"}
    failed: [clickhouse] (item=clickhouse-server) => {"ansible_loop_var": "item", "changed": false, "dest": "./clickhouse-server-22.9.7.34.rpm", "elapsed": 0, "gid": 1000, "group": "centos", "item": "clickhouse-server", "mode": "0644", "msg": "Request failed", "owner": "centos", "response": "HTTP Error 404: Not Found", "secontext": "unconfined_u:object_r:user_home_t:s0", "size": 114011, "state": "file", "status_code": 404, "uid": 1000, "url": "https://packages.clickhouse.com/rpm/stable/clickhouse-server-22.9.7.34.noarch.rpm"}
    failed: [clickhouse] (item=clickhouse-common-static) => {"ansible_loop_var": "item", "changed": false, "dest": "./clickhouse-common-static-22.9.7.34.rpm", "elapsed": 0, "gid": 1000, "group": "centos", "item": "clickhouse-common-static", "mode": "0644", "msg": "Request failed", "owner": "centos", "response": "HTTP Error 404: Not Found", "secontext": "unconfined_u:object_r:user_home_t:s0", "size": 248224665, "state": "file", "status_code": 404, "uid": 1000, "url": "https://packages.clickhouse.com/rpm/stable/clickhouse-common-static-22.9.7.34.noarch.rpm"}
    ...
    PLAY RECAP ****************************************************************************************************************************************************************
    clickhouse                 : ok=8    changed=0    unreachable=0    failed=0    skipped=0    rescued=1    ignored=0   
...
9. Подготовьте README.md файл по своему playbook. В нём должно быть описано: что делает playbook, какие у него есть параметры и теги.
    ...
    **Результат:** [README.md](https://github.com/ivanmanokhin/cvl-stack/blob/main/README.md)

10. Готовый playbook выложите в свой репозиторий, поставьте тег `08-ansible-03-yandex` на фиксирующий коммит, в ответ предоставьте ссылку на него.

    **Результат:** [репозиторий](https://github.com/ivanmanokhin/cvl-stack/tree/08-ansible-03-yandex)
